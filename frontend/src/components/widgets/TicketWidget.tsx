...
import { 
  ...
  Upload, Bell, KeyRound, HelpCircle
} from 'lucide-react';
...
export const TicketWidget = () => {
  ...
  const handleResolve = async (id: string, e?: React.MouseEvent) => {
    ...
    try {
      ...
      if (showThread && showThread.id === id) {
        setShowThread({ ...showThread, status: 'RESOLVED' });
      }
      fetchTickets();
    } catch (error) {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 pr-2">
                    ...
                    <p className="text-[10px] font-semibold text-slate-400">{ticket.ticketNumber} · {ticket.category} {isIT && ticket.submittedBy?.name ? `· ${ticket.submittedBy.name}` : ''}</p>
                  </div>
                  
                  {isIT && (ticket.status === 'OPEN' || ticket.status === 'IN_PROGRESS') ? (
                    <button 
                      onClick={(e) => handleResolve(ticket.id, e)}
                      className="p-1.5 shrink-0 rounded-lg bg-emerald-50 text-emerald-600 hover:bg-emerald-100 hover:text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20 transition-colors opacity-0 group-hover:opacity-100"
                      title="Quick Resolve"
                    >
          ...
          {showThread && (
            ...
            >
              ...
              >
                ...
                {/* IT Action Bar */}
                {isIT && showThread.status !== 'RESOLVED' && showThread.status !== 'CLOSED' && (
                  <div className="px-6 py-3 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 shrink-0 flex justify-end">
                    <button
                      onClick={() => handleResolve(showThread.id)}
                      className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-xs font-bold shadow-sm shadow-emerald-500/20 flex items-center gap-2 transition-colors"
                    >
